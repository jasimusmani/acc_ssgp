...
def main(unused_argv):
  ...
  location_X_red = '/home/jasimusmani/Documents/Subspace_Graph_Physics_Main/subspace_data/X_reduced_2_5k.npy'
  

  
  with open(os.path.join(pca_load, 'pca_eigen_vectors_4_8k.pkl'), 'rb') as f:
    W = pickle.load(f)
    W = W[:, :MODE_NUMBER]
  ...
  for ax_i, (label, rollout_field) in enumerate(
    ...
    if FLAGS.fullspace:
      ...
      np.save('rollout_data_old.npy', Xr2D)
      print("file saved")
    #   X2D = np.matmul(Xr2D, np.transpose(W))

      # nystrom_kpca = NystromKPCA(n_components= 8 ,m_subset= 500, scale=False, kernel = 'rbf')
      # X2D = nystrom_kpca.fit_transform(Xr2D)

      # X2D = np.load(location_X_red)
