...
if __name__ == '__main__':
    ...
    dataset_save = dataset_load + 'test'


    # Shuffle and create train, valid, and test examples (cases)
    no_examples = 5  # 250
    no_train_examples = int(0.60 * no_examples)  # to train model
    no_valid_examples = int(0.20 * no_examples)  # to tune hyperparameters  
    no_test_examples  = int(0.20 * no_examples)  # to evaluate final model
                        ...
                        for angle in [0.0, 3.8, 10.0, 30.8, 45.0]:
                            for motion in [1.0, 2.0]:
                                ...
                                if dataset_split[split][c3] == c1:
                                    ...
                                    # Xr2D = np.matmul(X, W)
                                    raw_data = sklearn.decomposition.KernelPCA(n_components= 8,  kernel="poly",  gamma = 0.0391, fit_inverse_transform = True)
                                    Xr2D = raw_data.fit_transform(X)
                                    # np.save(f"X_red_{c2}", Xr2D)
                                    invmat = raw_data.dual_coef_  
                                    np.save(f"inv_mat_test_{c2}", invmat)
