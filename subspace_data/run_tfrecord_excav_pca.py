...
if __name__ == '__main__':
    ...
    dataset_save = dataset_load + 'PCA_6'
    pos_all = list()
    pos_rigid = list()
    vel_all = list()
    # Shuffle and create train, valid, and test examples (cases)
    no_examples = 250
    # no_train_examples = int(0.8 * no_examples)  # to train model
    # no_valid_examples = int(0.1 * no_examples)  # to tune hyperparameters  
    # no_test_examples  = int(0.1 * no_examples)  # to evaluate final model
    ...
    valid_example_list = [21]
    # test_example_list = [18, 16, 15, 24, 12]

    test_example_list = [240]
    dataset_split = {
    ...
    for counter, split in enumerate(dataset_split):
                        ...
                        for angle in [0.0, 3.8, 10.0, 30.8, 45.0]:
                            for motion in [1.0, 2.0]:
                                ...
                                if dataset_split[split][c3] == c1:
                                    ...
                                    for frame in range(1, frames+1):
                                        ...
                                        if frame == 1:
                                            ...
                                            rigid_body_force.append([
                                                ...
                                                round(float(data_frame['col1'][0])/(scale_dim**3), 6),
                                                round(float(data_frame['col2'][0])/(scale_dim**3), 6)])
                                            rigid_forces = np.asarray(rigid_body_force)
                                            # np.save("rigid_body_force", rigid_forces)        
                                            if VISUALIZATION:
                                                particle_type_np.append(np.array(particle_type_frame_np))
                                                particle_pos_np.append(np.array(particle_pos_frame_np))
                                        if frame % 10 == 0:
                                    ...
                                    # np.save("particle_pos.npy",particle_pos)
                                    ...
                                    Xr2D = np.matmul(X, W)
                                    # Apply Kernel PCA to 2D data 
                                    #added

                                    # X_base = np.load("X_data_tf.npy")
                                    # transformer = np.load("transformer.npy")
                                    # kernel = sklearn.metrics.pairwise.pairwise_kernels(X, X_base, metric='poly', gamma = 0.0391)
                                    # Xr2D = np.matmul(kernel, transformer)


                                    # raw_data = sklearn.decomposition.KernelPCA(n_components= 14,  kernel="rbf", gamma = 0.5, fit_inverse_transform = True)
                                    # Xr2D = raw_data.fit_transform(X)
                                    # np.save(f"X_red_sample_{c2}", Xr2D)
                                    # invmat = raw_data.dual_coef_  
                                    # np.save(f"inv_mat_sample_{c2}", invmat)

                                    print(Xr2D.shape)
                                    # pos_all.append(Xr2D)
                                    # Convert 2D data to 3D data
                                    Xr = np.zeros((frames+1, MODE_NUMBER+r_particles, dimension))
                                    # Xr = np.zeros((frames+1, MODE_NUMBER, dimension))
                                    for i in range(frames+1):
                                    ...
                                    print("four")
                                    # with open('rigid_body_force_bytes.pkl', 'wb') as f:
                                    #     pickle.dump(rigid_body_force_bytes, f)
                                    data = {
                                    ...
                                    if split == 'train':
                                        ...
                                        cou += 1
                                        # np.save(f"particle_vel.npy_{cou}",particle_vel)
                                        # vel_all.append(particle_vel)
                                        # Calculate mean and variance
                                        xvel = []
                                        yvel = []
                                        zvel = []
        ...
        # np.save(f"pos_all_{split}",pos_all)
        # pos_rigid=np.stack(pos_rigid)
        # np.save(f"pos_rigid_{split}",pos_rigid)
        # vel_all=np.stack(vel_all)
        # np.save(f"vel_all_{split}",vel_all)
