...
if __name__ == '__main__':
    ...
    dataset_save = dataset_load + '_PCA_G_1.62'

    pos_train_16 = list()
    pos_valid_16 = list()
    pos_test_16 = list()
    pos_train_not16 = list()
    pos_valid_not16 = list()
    pos_test_not16  = list()
    vel_train_16 = list()
    vel_train_not16  = list()
    vel_test_16 = list()
    vel_test_not16 = list()
    vel_valid_16 = list()
    vel_valid_not16 = list()
    ...
    for counter, split in enumerate(dataset_split):
        ...
        with tf.python_io.TFRecordWriter(tfr_filename) as writer:
            ...
            while c2 < len_split:
                ...
                # Creating TFRecord files for gravity = 1.6 case
                for gravity in [1.62]:
                            ...
                            for wdia in [0.05, 0.15, 0.30]:
                                for sfangle in [30, 37, 43]:
                                    ...
                                    if dataset_split[split][c3] == c1:
                                        ...
                                        if r_particles==16:             
                                            if split== "train":
                                                pos_train_16.append(Xr)
                                            if split== "test":
                                                pos_test_16.append(Xr)
                                            if split== "valid":
                                                pos_valid_16.append(Xr)
                                        if r_particles!=16:             
                                            if split== "train":
                                                pos_train_not16.append(Xr)
                                            if split== "test":
                                                pos_test_not16.append(Xr)
                                            if split== "valid":
                                                pos_valid_not16.append(Xr)
                                        ...
                                        if split == 'train':
                                            ...
                                            if r_particles==16:
                                                if split == "train":
                                                    vel_train_16.append(particle_vel)
                                                if split== "test":
                                                    vel_test_16.append(particle_vel)
                                                if split== "valid":
                                                    vel_valid_16.append(particle_vel)
                                            if r_particles!=16:
                                                if split == "train":
                                                    vel_train_not16.append(particle_vel)
                                                if split== "test":
                                                    vel_test_not16.append(particle_vel)
                                                if split== "valid":
                                                    vel_valid_not16.append(particle_vel)
        ...
        if split == "train":
            pos_train_16=np.stack(pos_train_16)
            np.save("pos_train_16",pos_train_16)
            pos_train_not16=np.stack(pos_train_not16)
            np.save("pos_train_not16",pos_train_not16)
        if split == "test":
            pos_test_16=np.stack(pos_test_16)
            np.save("pos_test_16",pos_test_16)
            pos_test_not16=np.stack(pos_test_not16)
            np.save("pos_test_not16",pos_test_not16)
        if split == "valid":
            pos_valid_16 = np.stack(pos_valid_16)
            np.save("pos_valid_16",pos_valid_16)
            pos_valid_not16 = np.stack(pos_valid_not16)
            np.save("pos_valid_not16",pos_valid_not16)


        if split == "train":
            vel_train_16=np.stack(vel_train_16)
            np.save("vel_train_16",vel_train_16)
            vel_train_not16=np.stack(vel_train_not16)
            np.save("vel_train_not16",vel_train_not16)
        if split == "test":
            vel_test_16=np.stack(vel_test_16)
            np.save(f"vel_test_16",vel_test_16)
            vel_test_not16=np.stack(vel_test_not16)
            np.save(f"vel_test_not16",vel_test_not16)
        if split == "valid":
            vel_valid_16 = np.stack(vel_valid_16)
            np.save(f"vel_valid_16",vel_valid_16)
            vel_valid_not16 = np.stack(vel_valid_not16)
            np.save(f"vel_valid_not16",vel_valid_not16)
