...
# from subspace_data import kernel_pca

if __name__ == '__main__':
    dataset_load = '/home/jasimusmani/Documents/Subspace_Graph_Physics_Main/learning_to_simulate/datasets/' + 'Excavation'  
    dataset_save = dataset_load + 'KPCA_selected'
            ...
            for angle in [0.0, 3.8, 10.0, 30.8, 45.0]:
                for motion in [1.0, 2.0]:
                    ...
                    if c == 240:
                        ...
                        # newpath = os.path.join(ds_path, 'frames')
                        # for (root,dirs,files) in os.walk(newpath, topdown=True):
                        #     print (root)
                        #     print (dirs)
                        #     print (files)

                        path, dirs, files = next(os.walk(os.path.join(ds_path, 'frames')))  #stopping here itself and not moving forward
    ...
    EigVec, EigVal, MEAN = kernel_pca.kpca(X)
