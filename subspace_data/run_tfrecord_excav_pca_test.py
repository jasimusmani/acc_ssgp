...
if __name__ == '__main__':
    ...
    dataset_save = dataset_load + '_PCA_6'
    pos_train_15 = list()
    pos_valid_15 = list()
    pos_test_15 = list()
    pos_train_20 = list()
    ...
    no_examples = 250  # 250
    no_train_examples = int(0.9 * no_examples)  # to train model
    ...
    # train_example_list1 = [248, 150, 132, 149, 97, 67, 132, 192, 94, 179, 112,
    #  18, 211, 146, 242, 183, 111, 177, 220, 90, 135, 63, 72, 247,
    #  179, 126, 162, 182, 131, 95, 165, 165,97, 20, 99, 223, 34, 186, 141, 165,
    #     79, 90, 93, 64, 66, 92, 126, 237, 34, 163, 121, 250, 125, 116, 192, 63, 12, 
    #   243, 85, 222, 239, 131, 45, 35, 161, 245, 171, 143, 75, 244, 179, 82, 135, 47, 188, 
    #   232, 43, 97, 236, 21, 111, 216, 200, 114, 99, 15, 244, 180]

    # train_example_list1 = [72, 62, 13, 241, 118, 47, 219, 217, 162, 170, 29, 94, 43, 68, 36, 148, 143, 
    # 142, 65, 123, 38, 43, 172, 86, 228, 212, 37, 239, 42, 150, 134, 245, 162, 42, 25, 83, 132, 229, 
    # 62, 19, 28, 16, 122, 86, 243, 118, 244, 84, 127, 48, 242, 81, 196, 249, 66, 82, 127, 140, 90, 
    # 127, 211, 194, 213, 239, 135, 42, 112, 33, 222, 244, 199, 97, 217, 17, 86, 92, 184, 198, 236, 
    # 42, 187, 146, 224, 226, 112, 212, 220, 76, 86, 185, 92, 127, 93, 65, 120, 63, 237, 18, 40]



    valid_example_list1 = [169, 130, 169, 74, 87, 11, 193, 11, 195, 49, 71]
    test_example_list1 = [24, 231, 235, 214, 89, 14, 113, 240, 230]


    train_example_list = [24, 231, 235, 214, 89, 14, 113, 240, 230]
    dataset_split = {
        'train': train_example_list,
        'valid': valid_example_list1,
        'test': test_example_list1
    }
            ...
            while c2 < len_split:
                ...
                for depth in [0.02, 0.04, 0.05, 0.08, 0.1]:
                        ...
                        for angle in [0.0, 3.8, 10.0, 30.8, 45.0]:
                            for motion in [1.0, 2.0]:
                                ...
                                if dataset_split[split][c3] == c1:
                                    ...
                                    Xr2D = np.matmul(X, W)
                                    # Xr2D = X
                                    print("Xr2D shape:", Xr2D.shape)
                                    # print(Xr2D.shape)
                                    # if split== "train":
                                    #     pos_train.append(Xr2D)
                                    # if split== "test":
                                    #     pos_test.append(Xr2D)
                                    # if split== "valid":
                                    #     pos_valid.append(Xr2D)
                                    ...
                                    if r_particles==15:             
                                        if split== "train":
                                            pos_train_15.append(Xr)
                                        if split== "test":
                                            pos_test_15.append(Xr)
                                        if split== "valid":
                                            pos_valid_15.append(Xr)
                                    if r_particles==20:             
                                        if split== "train":
                                            pos_train_20.append(Xr)
                                        if split== "test":
                                            pos_test_20.append(Xr)
                                        if split== "valid":
                                            pos_valid_20.append(Xr)
                                    ...
                                    print("particle 2d",particle_pos_2D.shape)
                                    particle_type_bytes = []
                                    particle_pos_bytes = []
                                    ...
                                    for frame in range(1, frames):
                                        # Particle velocity (including rigid particles)
                                        ...
                                        cc += 1
                                        if frame >= 2:
                                            # Particle acceleration (including rigid particles)
                                    ...
                                    if r_particles==15:
                                        if split == "train":
                                            vel_train_15.append(particle_vel)
                                        if split== "test":
                                            vel_test_15.append(particle_vel)
                                        if split== "valid":
                                            vel_valid_15.append(particle_vel)
                                    if r_particles==20:
                                        if split == "train":
                                            vel_train_20.append(particle_vel)
                                        if split== "test":
                                            vel_test_20.append(particle_vel)
                                        if split== "valid":
                                            vel_valid_20.append(particle_vel)
                                    # Calculate mean and variance
                                    xvel = []
                                    yvel = []
                                    zvel = []
                    ...
                    if c2 >= len_split:
                        break
        if split == "train":
            pos_train_15=np.stack(pos_train_15)
            np.save("pos_train_15",pos_train_15)
            pos_train_20=np.stack(pos_train_20)
            np.save("pos_train_20",pos_train_20)
        if split == "test":
            pos_test_15=np.stack(pos_test_15)
            np.save("pos_test_15",pos_test_15)
            pos_test_20=np.stack(pos_test_20)
            np.save("pos_test_20",pos_test_20)
        if split == "valid":
            pos_valid_15 = np.stack(pos_valid_15)
            np.save("pos_valid_15",pos_valid_15)
            pos_valid_20 = np.stack(pos_valid_20)
            np.save("pos_valid_20",pos_valid_20)


        if split == "train":
            vel_train_15=np.stack(vel_train_15)
            np.save("vel_train_15",vel_train_15)
            vel_train_20=np.stack(vel_train_20)
            np.save("vel_train_20",vel_train_20)
        if split == "test":
            vel_test_15=np.stack(vel_test_15)
            np.save(f"vel_test_15",vel_test_15)
            vel_test_20=np.stack(vel_test_20)
            np.save(f"vel_test_20",vel_test_20)
        if split == "valid":
            vel_valid_15 = np.stack(vel_valid_15)
            np.save(f"vel_valid_15",vel_valid_15)
            vel_valid_20 = np.stack(vel_valid_20)
            np.save(f"vel_valid_20",vel_valid_20)
